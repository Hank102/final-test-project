...
class ProductPage(BasePage):
    def guest_can_add_product_to_basket(self): #добавить товар в корзину
        button = self.browser.find_element(*ProductPageLocators.ADD_TO_BASKET)
        button.click()

    def guest_can_see_product_name(self): #посмотреть название продукта
    ...
    def check_message_about_adding_to_basket(self): #сообщение о добавлении в корзину
        basket_message = self.browser.find_element(*ProductPageLocators.SUCCESS_MESSAGE)
        assert self.is_not_element_present(*ProductPageLocators.SUCCESS_MESSAGE), \
            "Success message is presented, but should not be"

    def check_success_message_on_product_page(self): #сообщение об успехе на странице продукта
        assert self.is_not_element_present(*ProductPageLocators.SUCCESS_MESSAGE), \
            "Success message is presented, but should not be"
